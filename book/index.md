# CI0202 — Principios de Informática

*Este sitio no reemplaza a Mediación Virtual de la UCR, si no que complementa el contenido teórico presentado. El sitio oficial del curso es Mediación Virtual*.

---

> *"La programación no es solo escribir código. Es aprender a pensar con claridad, resolver problemas con creatividad y construir ideas que antes solo existían en tu mente."*

---

## Bienvenida

Este es el libro digital del curso **CI0202 – Principios de Informática**, un curso introductorio diseñado para estudiantes que se adentran por primera vez en el mundo de la programación y la ciencia de la computación.

A lo largo de este curso se aprenderá de los fundamentos del pensamiento computacional y los principios básicos de la programación usando **Python**, uno de los lenguajes más populares, versátiles y amigables del mundo.

---

## Contenidos del sitio

Este sitio está organizado en los siguientes módulos:

```{tableofcontents}
```

---

## Descargar el material

¿Prefiere tener todas las lecciones en su computadora? Descargue el material teórico completo (notebooks, Markdown e imágenes) en un solo archivo comprimido.

{button}`Descargar todo el material teórico (.zip) <resources/material_teorico.zip>`

---

## Información de contacto

Los contenidos de este sitio fueron hechos por:

> Enrique Guillermo Vílchez Lizano,  
Profesor,  
Universidad de Costa Rica,  
Ciudad Universitaria Rodrigo Facio,  
San Pedro de Montes de Oca,  
Costa Rica.

y

> Antonio Badilla Olivas,  
Ex-Profesor,  
Universidad de Costa Rica,  
Ciudad Universitaria Rodrigo Facio,  
San Pedro de Montes de Oca,  
Costa Rica.

<a rel="license" href="http://creativecommons.org/licenses/by-sa/4.0/"><img alt="Licencia de Creative Commons" style="border-width:0" src="https://i.creativecommons.org/l/by-sa/4.0/88x31.png" /></a><br />Los contenidos de este curso, a menos que se especifique de otra forma, se comparten mediante una <a rel="license" href="http://creativecommons.org/licenses/by-sa/4.0/">licencia de Creative Commons Reconocimiento-CompartirIgual 4.0 Internacional</a>.

---

*Última actualización: 2026 · CI0202 Principios de Informática*  
*Escuela de Ciencias de la Computación e Informática · Universidad de Costa Rica*
